...
# Jarayondan rasmlar tugmasi (Tezkor xabar bilan)
@dp.message(F.text == "🖼 Jarayondan rasmlar")
async def photos_handler(message: Message):
    # Avval darhol kutish xabarini chiqaramiz
    wait_msg = await message.answer("⏳ Rasmlar yuklanmoqda, iltimos kuting...")
    
    try:
        media = [
            InputMediaPhoto(media=FSInputFile("suvoq1.png"), caption="Suvoq apparati yordamida ish jarayoni"),
            InputMediaPhoto(media=FSInputFile("suvoq2.png")),
            InputMediaPhoto(media=FSInputFile("suvoq3.png"))
        ]
        await message.answer_media_group(media=media)
        # Rasmlar chiqqandan keyin kutish xabarini o'chirib tashlaymiz
        await wait_msg.delete()
    except Exception as e:
        await wait_msg.edit_text("❌ Rasmlarni yuklashda xatolik yuz berdi. Fayl nomlarini tekshiring!")
